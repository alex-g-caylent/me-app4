...
const EditArticleForm = ({
	...
	const [formData, setFormData] = useState({
		...
		// cover: article?.cover || "",
		relevance: article?.jobTitleSkillRelevance
			? jobTitleSkills.map((skill) => {
					const relevance = article.jobTitleSkillRelevance.find(
						(r) => r.jobTitleSkillId === skill.id
					);
					return relevance ? relevance.relevance : 3;
			  })
			: jobTitleSkills.map(() => 3),
	});
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		try {
			...
			const jobTitleSkillsData = jobTitleSkills.map((skill, index) => ({
	...
	return (
		<form onSubmit={handleSubmit} className="space-y-4">
			...
			<div className="space-y-2">
				<label htmlFor="educationalMethodologyId" className="block font-bold">
			...
			<div className="space-y-2">
				<label htmlFor="educationalFrameworkId" className="block font-bold">
			...
			<div className="space-y-2">
				...
				<div className="space-y-1">
					{businessUnits.map((bu) => {
						...
						return (
							<label key={bu.id} className="flex items-center space-x-2">
								<input
									type="checkbox"
									checked={isSelected}
									onChange={(e) =>
			...
			<div className="space-y-4">
				<span className="block font-bold">Relevance per Job Title</span>
				{(jobTitleSkills || []).map((skill, index) => (
					<div key={skill.id} className="space-y-2">
						<span className="block font-semibold">
							{skill.jobTitle?.name || "Unknown Job Title"}
						</span>
						<div className="space-y-1">
							<div className="flex justify-between items-center">
								<span>{skill.skill?.name || "Unknown Skill"}</span>
								<select
									value={formData.relevance[index] || 3}
									onChange={(e) => {
										const newRelevance = [...(formData.relevance || [])];
										newRelevance[index] = Number(e.target.value);
										setFormData((prev) => ({
											...prev,
